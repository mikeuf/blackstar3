...
function ValidateForm()
{
  ...
  var SpamCheck = document.forms["form1"]["SpamCheck"].value;

 var Message = document.forms["form1"]["Message"].value;
  if (!ValidateNotBlank(Message,"message")) return false;

  if (!ValidateNotBlank(SpamCheck, "answer to the spam check question" )) return false;

  if (!ValidateSpamCheck(SpamCheck)) return false;
...
function ValidateDate(month, day, year)
{
  ...
  {
  alert("You must be at least 13 years old to use this form.");
  return false;
  }
...
function ValidateSpamCheck(val)
{
if (val != 1492)
{
alert("Spam check answer is incorrect. Try again.");
return false;
}
return true;
